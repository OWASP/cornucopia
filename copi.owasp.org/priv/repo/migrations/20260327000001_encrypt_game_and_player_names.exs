defmodule Copi.Repo.Migrations.EncryptGameAndPlayerNames do
  ...
  defp encrypt_existing_rows(table) do
    rows = repo().all(from r in table, select: {r.id, r.name})
    ...
    Enum.each(rows, fn {id, plaintext} ->
      case Copi.Encrypted.Binary.encrypt(plaintext) do
        ...
        {:error, reason} ->
          raise "Encryption failed for #{table} id=#{id}: #{inspect(reason)}"
      end
    end)
  end
  ...
  defp decrypt_existing_rows(table) do
    ...
    Enum.each(rows, fn {id, ciphertext} ->
      case Copi.Encrypted.Binary.decrypt(ciphertext) do
        ...
        {:error, reason} ->
          raise "Decryption failed for #{table} id=#{id}: #{inspect(reason)}"
      end
    end)
  end
end
