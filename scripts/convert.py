...
class ConvertVars:
    ...
    EDITION_CHOICES: List[str] = ["all", "webapp", "mobile"]
...
def parse_arguments(input_args: List[str]) -> argparse.Namespace:
    ...
    group.add_argument(
        ...
        help=(
            ...
            "`mobile` will give you the MASVS/MASTG edition."
        ),
    )
...
def get_suit_tags_and_key(key: str, edition: str) -> Tuple[List[str], str]:
    ...
    if key == "suits" and edition == "webapp":
        ...
        suit_key = "cards"
    if key == "suits" and edition == "mobile":
...
def has_not_valid_file_style(style: str, edition: str, file_type: str) -> bool:
    if (style == "leaflet" or edition == "mobile") and file_type != "idml":
        return True
    return False
