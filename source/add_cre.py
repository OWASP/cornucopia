        ...
        if len(results) > 0:
            for standard in body.get("standards"):
                ...
                if "links" in standard:
                    cres = [link.get("document").get('id') for link in standard.get("links") if link.get("document").get("doctype") == "CRE"]
                    if cres:
                        standard_to_cre[section] = cres
                    else:
                        pprint(f"{standard_name} {section} has links that aren't cre? looks like a bug!")
                        pprint(body)
                else:
...
for src in source:
    for suit in src.get('suits'):
        for card in suit.get("cards"):
            card_val_to_desc[card.get("value")] = card.get("desc")
