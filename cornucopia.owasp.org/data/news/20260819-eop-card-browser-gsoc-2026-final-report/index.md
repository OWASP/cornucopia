---
date: 20260819
author: ayman-algamal
tags: gsoc,gsoc2026,owasp,cornucopia,eop,threat-modeling
hidden: false
description: How the Elevation of Privilege (EoP) game was added to Cornucopia's card browser and API during GSoC 2026.
---
...
OWASP Cornucopia converts essential security concepts into a gamified format where
...
| [#3331](https://github.com/OWASP/cornucopia/pull/3331) | Follow-ups & Suggestions | Refactoring follow-ups from code review | Draft |

## Screenshots

### Browsing the EoP deck

The suit tree on the left lists all EoP suits; selecting one expands its cards
and shows a preview on the right.

![EoP suit tree with a Spoofing card preview](eop-suit-tree-and-card-preview.png)

Expanding a suit reveals its individual cards, like Information Disclosure here.

![EoP suit tree expanded to the Information Disclosure suit](eop-tree-expanded-information-disclosure.png)

Each card also has its own detail view, with previous/next navigation through the suit.

![A single EoP card in the detail/carousel view](eop-card-detail-repudiation-q.png)
...
![A grid of EoP Spoofing cards, each with a "Need more info?" link](eop-cards-grid-need-more-info-links.png)

### EoP in the API

The `GET /cre/eop/{lang}` endpoint serves EoP cards the same way the other editions are served.

![Swagger UI for the GET /cre/eop/{lang} endpoint](eop-api-endpoint-swagger.png)

![Example JSON response from the EoP API endpoint](eop-api-response-example.png)
