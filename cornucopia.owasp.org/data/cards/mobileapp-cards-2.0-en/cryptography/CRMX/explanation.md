...
Kouti finds an API token hard-coded in a cooking app’s bundled recipe for “secret sauce.” A curious diner extracts the package, uses the token to order ingredients, and sends 300 jars of mustard to Kouti’s apartment.

Keys must not live in source, bundles, or ordinary shared storage. Platform-protected keystores and secure secret handling reduce the chance that extracting app files exposes credentials or sensitive data.
...
This scenario is primarily **Tampering**, **Information Disclosure** in STRIDE. The named condition is: Kouti can extract cryptographic keys or sensitive data because they are hard-coded or stored without encryption or appropriate platform protection in app-private or shared storage.
...
- **MAS-THREAT-0003:** Attackers can extract cryptographic keys stored outside the platform keystore.
...
If Kouti can extract cryptographic keys or sensitive data because they are hard-coded or stored without encryption or appropriate platform protection in app-private or shared storage, the failure is concrete rather than merely theatrical: the app could let an attacker cross the cryptography boundary and reach data or capability that this flow should protect. In this card, the practical route includes Accessing shared or external storage from any app holding the corresponding permissions. Also, Physically removing and reading external storage media such as SD cards. That can turn a normal user action into unauthorized access, disclosure, alteration, or service disruption; the mapped weakness entries below identify the exact implementation evidence to check.
...
- MASWE-0003 — Cryptographic Keys Stored Outside of Platform Keystore: This weakness occurs when cryptographic keys are stored outside the platform-provided secure keystore, in locations such as unencrypted preferences, unprotected files, or the application code itself.

### What are we going to do about it?

Never hard-code keys or leave them in app-private or shared files; generate and use them through Android Keystore or iOS Keychain, rotate compromised material, and test the package and extracted storage for recoverable secrets.
...
- MASTG-TEST-0212 — Use of Hardcoded Cryptographic Keys in Code: In this test case, we will look for the use of hardcoded keys in Android applications. To do this, we need to focus on the cryptographic implementations of hardcoded keys. The Java Cryptography Architecture (JCA) provides the...
- MASTG-TEST-0213 — Use of Hardcoded Cryptographic Keys in Code: In this test case, we will examine iOS applications for the presence of hardcoded cryptographic keys. Hardcoded keys can be typically found in calls to cryptographic functions or stored as constants or variables within the code. In iOS,...
- MASTG-TEST-0214 — Hardcoded Cryptographic Keys in Files: Cryptographic keys may be embedded files such as configuration files or key files, certificate files, or other resource files bundled with the app, making them accessible to anyone who can extract the app's resources. Real-world cases...
