...
Carlos receives a delivery notification saying, “Package held—tap to pay at this link.” The notification’s button opens a look-alike payment page, and his phone’s parcel tracker has effectively become a tiny pickpocket with excellent timing.

Notification text and destinations must be sanitized and allowlisted rather than treated as harmless decoration. Otherwise an attacker can turn the app’s trusted alert channel into a convincing phishing campaign.
...
This scenario is primarily **Tampering**, **Denial of Service** in STRIDE. The named condition is: Carlos can use the application's notification services to launch phishing campaigns because notification content or action destinations are not appropriately sanitized or validated.
...
If Carlos can use the application's notification services to launch phishing campaigns because notification content or action destinations are not appropriately sanitized or validated, the failure is concrete rather than merely theatrical: the app could let an attacker cross the cornucopia boundary and reach data or capability that this flow should protect. In this card, the practical route includes Registering the same custom URL scheme to intercept links intended for the app. Also, Delivering crafted deep links or intents from a malicious app or web page. That can turn a normal user action into unauthorized access, disclosure, alteration, or service disruption; the mapped weakness entries below identify the exact implementation evidence to check.
...
Treat notification text and actions as attacker-visible input: redact sensitive content, allow only verified destinations and safe schemes, and validate notification deep links before opening them; test phishing-style content, redirects, and action handling.
