...
Aatif helps install a smart gate opener app for a customer so that the owner can open the gate to their property by using their face or fingerprint. What the customer doesn't know is that the app verifies the biometric locally and sends a simple, un-signed local command to the gate saying auth_status=success and time_of_entry=(time of entry) and Aatif, having done some "Wiresharking" during the installation, knows exactly the right request to send to the gate. At night, Aatif replays the command by changing the time_of_entry parameter to now and sending it to the gate's receiver, bypassing the phone's biometric control entirely.

## Threat Modeling

### STRIDE

This scenario covers 4 of the STRIDE categories: **Spoofing**, **Tampering**, **Repudiation** and **Information Disclosure**.
...
Because Aatif can influence or alter authentication controls and bypass them, the app lets an attacker break onto the property without proper authorization and without being detected.
