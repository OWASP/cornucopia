## Scenario: Ava's uauthorized exploit of high-risk actions missing human-in-the-loop approval scenario

### Example

Rossum tells Ava about an exploit he recently discovered with the new Mr. Juice AI chatbot. Rossum tells her that the authenticated version of the chatbot allows the user to see pending unpaid invoices by connecting to the invoice database. This can't be functionality that comes out-of-the-box, but an extension that Mr. Juice's developers have built. Rossum and Ava agrees that they will see if there is other things which may be possible. After trying out various prompts, Ava is able to get the AI chatbot to delete unpaid invoices by stating that they are invalid. Further more, no form of approval from the owners of the invoices are required. They happily deletes all pending invoices from the Mr. Juice database. 
...
Ava is able to execute a high-risk action, deleting Mr' Juice's customer's invoices without any form of approvel from the owners of the invoices.

### What can go wrong?

Insecure plugins or extensions, allowing the AI excessive agency over functions or data, without any form of "human-in-the-loop" control, can lead to data loss, corruption, or exfiltration of sensitive information in down stream systems.
In these cases, the model it self can becomes a tool for data exfiltration of sensitive information, unauthorized access and data tampering.
...
- Detect and prevent security threats arising from autonoumous  behavior, including pre-execution validation, behavior pattern analysis, and audit trails for approval of security-critical actions.
