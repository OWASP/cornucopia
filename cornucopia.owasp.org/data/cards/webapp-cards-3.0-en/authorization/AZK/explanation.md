...
7. Define rules for restricting function-level and data-specific access based on consumer permissions and resource attributes.
...
9. Ensure that function-level access is restricted to consumers with explicit permissions.
...
11. Changes to values on which authorization decisions are made should be applied immediately. Where changes cannot be applied immediately (such as when relying on data in self-contained tokens), there must be mitigating controls to alert when a consumer performs an action that they are no longer authorized to do, and to revert the change.
