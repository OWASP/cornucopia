        ...
        <div class="tree">
            ...
            {#if version == VERSION_WEBAPP}
                ...
                {#each webappSuits as suit (suit.name)}
                    ...
                    <h3 title="OWASP Cornucopia {Text.Format(suit.name).toUpperCase()} suit" onkeypress={()=>toggle(suit.name)} onclick={()=>toggle(suit.name)}>ÃŽâ€œÃƒÂ¶ÃƒÂ¶ÃŽâ€œÃƒÂ¶Ãƒâ€¡ÃŽâ€œÃƒÂ¶Ãƒâ€¡ {Text.Format(suit.name).toUpperCase()}</h3>
                    {#if map?.get(suit.name)}
                        {#each suit.cards as card (card)}
                            <p onmouseenter={()=>{enter(suit.name, cards?.get(card)?.id)}}>
                                <a title="OWASP Cornucopia {Text.Format(suit.name).toUpperCase()}, {cards?.get(card)?.id}" href={cards?.get(card)?.url ?? ''}>ÃŽâ€œÃƒÂ¶Ã‚Â£ÃŽâ€œÃƒÂ¶Ãƒâ€¡ÃŽâ€œÃƒÂ¶Ãƒâ€¡ {cards?.get(card)?.id}</a>
                            </p>
                        {/each}
                    {/if}
                {/each}
...
<noscript>
    <div class="">
        <div>
            ...
            {#each webappSuits as suit (suit.name)}
                <label for="{suit.name + '-web'}" class="suit-button"><span class="label">ÃŽâ€œÃƒÂ¶ÃƒÂ¶ÃŽâ€œÃƒÂ¶Ãƒâ€¡ÃŽâ€œÃƒÂ¶Ãƒâ€¡ {Text.Format(suit.name).toUpperCase()}</span></label>
                <input type=checkbox class="suit-button" id="{suit.name + '-web'}"/>
                <div class="card-buttons">
                {#each suit.cards as card (card)}
                    <p>
                        <a title="OWASP Cornucopia card: {cards?.get(card)?.id} from suit: {Text.Format(suit.name).toUpperCase()}" href={cards?.get(card)?.url ?? ''}>ÃŽâ€œÃƒÂ¶Ã‚Â£ÃŽâ€œÃƒÂ¶Ãƒâ€¡ÃŽâ€œÃƒÂ¶Ãƒâ€¡ {cards?.get(card)?.id}</a>
                    </p>
                {/each}
                </div>
            {/each}
    ...
    <div class="">
        <div>
            ...
            {#each mobileappSuits as suit (suit.name)}
                <label for="{suit.name + '-mobile'}" class="suit-button"><span class="label">ÃŽâ€œÃƒÂ¶ÃƒÂ¶ÃŽâ€œÃƒÂ¶Ãƒâ€¡ÃŽâ€œÃƒÂ¶Ãƒâ€¡ {Text.Format(suit.name).toUpperCase()}</span></label>
    ...
    <div class="">
        <div>
            ...
            {#each companionSuits as suit (suit.name)}
                <label for="{suit.name + '-companion'}" class="suit-button"><span class="label">ÃŽâ€œÃƒÂ¶ÃƒÂ¶ÃŽâ€œÃƒÂ¶Ãƒâ€¡ÃŽâ€œÃƒÂ¶Ãƒâ€¡ {Text.Format(suit.name).toUpperCase()}</span></label>
